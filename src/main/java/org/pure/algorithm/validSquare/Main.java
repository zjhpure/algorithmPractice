...
public class Main {
    ...
    // 方法2，错误，通过判断一个点和其他三个点的距离是否能构成三角形，但是这里进行了开根号，开根号后精度丢失，不可能出现相等，而且也可能是长方形
    ...
    // 方法3，错误，和方法2的思路一样，但是改进了方法2，不开根号，精度不丢失，判断了两点不能重复，但是还是错误的，因为没有考虑另外一种情况，当三个点在一条直线上，例如：[0,1],[1,2],[0,2],[0,0]
    private boolean validSquare3(int[] p1, int[] p2, int[] p3, int[] p4) {
        // p1和p2之间的距离的平方，根据公式，a点和b点的距离 = (a^2 + b^2)的开根号
        double p12 = Math.pow((p1[0] - p2[0]), 2) + Math.pow((p1[1] - p2[1]), 2);
    ...
    // 方法4，错误，调整方法3，去掉方法3的用另一条交叉线代替最后两点的距离
    private boolean validSquare4(int[] p1, int[] p2, int[] p3, int[] p4) {
        // p1和p2之间的距离的平方，根据公式，a点和b点的距离 = (a^2 + b^2)的开根号
        double p12 = Math.pow((p1[0] - p2[0]), 2) + Math.pow((p1[1] - p2[1]), 2);
    ...
    // 方法5，正确，调整方法4，判断任意三个点是否构成等腰直角三角形
    private boolean validSquare5(int[] p1, int[] p2, int[] p3, int[] p4) {
        // 判断任意三个点是否构成等腰直角三角形
